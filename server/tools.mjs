// Harness — Lean AI OS · MCP tool catalog
// The TOOLS array (thin wrappers over src/engine.mjs). Kept separate from the JSON-RPC
// transport (mcp.mjs) so each file stays small and single-purpose (ADR-0035).

import * as engine from "../src/engine.mjs";

export const J = (o) => JSON.stringify(o, null, 2);
import { TOOLS_EXT } from "./tools-ext.mjs";
...
export const TOOLS = [
  ...
  {
    name: "os_validate",
    description: "VALIDATION LOOP (ADR-0036): runs the project's checks (test/lint/typecheck/build or an explicit command), captures failures, appends them to the errors-log and returns a structured summary. The LLM reads the failures, fixes the code and calls os_validate again until it passes. Harness runs+logs; the LLM fixes. Params: kind (test|lint|typecheck|build, default test), command (explicit override).",
    inputSchema: { type: "object", properties: { kind: { type: "string", enum: ["test", "lint", "typecheck", "build"] }, command: { type: "string", description: "explicit command override, e.g. 'pytest -q'" } } },
    run: ({ kind, command }) => J(engine.validate({ kind: kind || "test", command: command || null })),
  },
  {
    name: "os_checks",
    description: "Lists which validation checks are runnable now (matching scripts in the project's package.json). Use before os_validate to know what exists.",
    inputSchema: { type: "object", properties: {} },
    run: () => J(engine.detectChecks()),
  },
  {
    name: "os_metrics",
