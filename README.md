...
3. Escolha o arquivo **`extension/harness-lean-ai-os-0.3.1.vsix`** (dentro da pasta do Harness).
4. Pronto — o ícone do Harness aparece na barra lateral.

> Se você não vê esse arquivo, é porque seu projeto não tem a pasta `extension/` ainda. Rode `npx @igorkadu/harness scaffold .` (ou `upgrade .`) primeiro — isso copia a extensão para o seu projeto.
...
## Licença
MIT — use, faça fork, publique. Versão atual: **v0.3.1**.
