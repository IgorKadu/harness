...
git add -A
git commit -m "feat: orquestrador v0.4.1 + MCP 23 tools + extensao + painel web + upgrade-safe (preserva memoria)"
git branch -M main
...
```bash
npm i -g @vscode/vsce
cd extension && vsce package           # gera harness-lean-ai-os-0.4.1.vsix
